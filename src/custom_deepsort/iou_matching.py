...
def iou_distance(track_boxes: List[List[float]], detection_boxes: List[List[float]]) -> np.ndarray:
    if len(track_boxes) == 0 or len(detection_boxes) == 0:
        return np.empty((len(track_boxes), len(detection_boxes)), dtype=np.float32)

    cost = np.zeros((len(track_boxes), len(detection_boxes)), dtype=np.float32)
